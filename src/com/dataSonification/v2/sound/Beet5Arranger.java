...
/**
 * Arranges DataEvents from a MovementAnalyzer in terms of a Beethoven 5th Symphony Motive
 * @author Edward Childs
 *
 * @see MovementAnalyzer
 */
class Beet5Arranger extends Arranger
{
...
//	 package-private access
            Sonifiable arrange(Map<Key,Object> table)
            {
                ...
                int second_pitch = base_pitch
                        + (((increment - last_increment) > 0) ? Const.BEET_5_INTERVAL: -1 * Const.BEET_5_INTERVAL);
                int NREPEATS = Const.BEET_5_REPEATS;
                for (int i = 0; i < NREPEATS; i++)
                {
                    music.add(duration, base_pitch, loudness, hold);
                }
                music.add(duration*Const.BEET_5_MULT,second_pitch,loudness*Const.BEET_5_MULT, hold*Const.BEET_5_MULT);
